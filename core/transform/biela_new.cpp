...
static size_t newUtf8Len(unsigned char lead) {
    if (lead < 0x80) {
        return 1;
    }
    if ((lead >> 5) == 0x6) {
...
static bool newDecodeUtf8(const std::string& s, size_t pos, char32_t& cp, size_t& len) {
...
static bool newIsBoundary(char32_t cp) {
    if (cp < 0x80) {
...
static bool newIsVowel(char32_t cp) {
    switch (cp) {
    case 0x0410:
    case 0x0430:
    case 0x0415:
...
void bielaNewLatin::transform() {
    ...
    for (size_t i = 0; i < this->input.size(); ) {
        char32_t cp = 0;
        size_t len = 1;
        newDecodeUtf8(this->input, i, cp, len);
        ...
        char32_t nextCp = 0;
        size_t nextLen = 0;
        const bool hasNext = i + len < this->input.size() && newDecodeUtf8(this->input, i + len, nextCp, nextLen);
        ...
        case 0x0412: out += "V"; prevWasVowel = false; atWordStart = false; break;
        case 0x0432: out += "v"; prevWasVowel = false; atWordStart = false; break;
        ...
        case 0x041B: out += "L"; prevWasVowel = false; atWordStart = false; break;
        case 0x043B: out += "l"; prevWasVowel = false; atWordStart = false; break;
        ...
        case 0x0427: out += "\xC4\x8C"; prevWasVowel = false; atWordStart = false; break;
        case 0x0447: out += "\xC4\x8D"; prevWasVowel = false; atWordStart = false; break;
        case 0x0428: out += "\xC5\xA0"; prevWasVowel = false; atWordStart = false; break;
        case 0x0448: out += "\xC5\xA1"; prevWasVowel = false; atWordStart = false; break;
